...
def get_knn(ref_embeds, embeds, k, embeds_same_source=False, device_ids=None):
    ...
    if device_ids is not None:
        ...
        # 使用 index.search 方法来搜索查询数据集 embeds 中与参考数据集 ref_embeds 之间的 k+1 个最近邻
        if embeds_same_source:
...
def evaluate_cos(model, dataloader):
    ...
    with torch.no_grad():
        ...
        # X：[sample_num, 512], T:[sample_num]
        # print(f"X shape is {X.shape}, T {T.shape}")
        X = l2_norm(X)
        ...
        # 各个label在target中出现的次数{label:count, ...}，num_k:出现最多的次数，即count的最大值
        # print(f"label_counts is {label_counts}, num_k is {num_k}")
        knn_indices, knn_distances = get_knn(feature_coll, feature_coll, num_k, True, None)
        # knn_indices:[sample_num, num_k]代表每一条数据的num_k个最近邻的索引
        # print(f"knn_indices is {knn_indices}, shape is {len(knn_indices)}, {len(knn_indices[0])}, num is {T.shape}")
        knn_labels = target_labels[knn_indices]
        # knn_labels：[sample_num, num_k]代表每一条数据的num_k个最近邻的标签（类别，家庭）
        # print(f"knn_labels is {knn_labels}")
        # exit(1)
...
def new_evaluate_cos(model, dataloader):
    torch.cuda.empty_cache()
    ...
    with torch.no_grad():
        ...
        # 各个label在target中出现的次数{label:count, ...}，num_k:出现最多的次数，即count的最大值
        # print(f"label_counts is {label_counts}, num_k is {num_k}")
        knn_indices, knn_distances = get_knn(feature_coll, feature_coll, 5, True, None)
        # knn_indices:[sample_num, num_k]代表每一条数据的num_k个最近邻的索引
        # print(f"knn_indices is {knn_indices}, shape is {len(knn_indices)}, {len(knn_indices[0])}, num is {T.shape}")
        knn_labels = target_labels[knn_indices]
        correct_labels = (knn_labels == (np.tile(T.reshape(-1, 1), (1, 5))))
        total = correct_labels.shape[0]
        accuracies = []
        for i in range(1,6):
            count = correct_labels[:,:i].sum()
            accuracies.append(count/total * 100)
        return accuracies
